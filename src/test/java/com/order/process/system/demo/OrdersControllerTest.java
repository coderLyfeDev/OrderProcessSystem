...
class OrdersControllerTest {
    ...
    @Test
    void createOrder_ServiceThrowsException_HandlesGracefully() {
        // Arrange
        CreateOrderRequest request = new CreateOrderRequest();
        request.setCustomerId(123);
        request.setItems(Arrays.asList(new ItemRequest(1L, 5)));
        ...
        assertThrows(RuntimeException.class, () -> {
            ordersController.createOrder(request);
        });
    ...
    @Test
    void getOrderStatus_ServiceReturnsNull_HandlesGracefully() {
        // Arrange
        Long orderId = 1L;
        OrderStatusResponse response = new OrderStatusResponse(null, "Error occurred");

        when(orderService.findOrderById(orderId)).thenReturn(response);

        // Act
        ResponseEntity<OrderStatusResponse> result = ordersController.getOrderStatus(orderId);

        // Assert
        assertEquals(HttpStatus.NOT_FOUND, result.getStatusCode());
        assertNull(result.getBody().getOrder());

        verify(orderService, times(1)).findOrderById(orderId);
    }

    @Test
    void verifyControllerAnnotations() {
        // Verify controller has correct annotations
        assertTrue(OrdersController.class.isAnnotationPresent(RestController.class));
        assertTrue(OrdersController.class.isAnnotationPresent(RequestMapping.class));
        assertTrue(OrdersController.class.isAnnotationPresent(CrossOrigin.class));

        RequestMapping requestMapping = OrdersController.class.getAnnotation(RequestMapping.class);
        assertEquals("/order", requestMapping.value()[0]);
    }
}
